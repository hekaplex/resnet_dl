...
MPI_NUM_PROCESSES=${MPI_NUM_PROCESSES:=4}
...
if [ -z "${BACKBONE_MODEL_DIR}" ]; then
  echo "The required environment variable BACKBONE_MODEL_DIR has not been set"
  exit 1
fi

if [ ! -d "${BACKBONE_MODEL_DIR}" ]; then
  echo "The BACKBONE_MODEL_DIR '${BACKBONE_MODEL_DIR}' does not exist"
  exit 1
fi
...
_command python ${MODEL_DIR}/benchmarks/launch_benchmark.py \
  ...
  --backbone-model ${BACKBONE_MODEL_DIR}
