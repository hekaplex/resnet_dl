...
class ModelInitializer(BaseModelInitializer):
    ...
    def __init__(self, args, custom_args=[], platform_util=None):
        ...
        arg_parser.add_argument('--train-option', help=' Training SQuAD, Pretraining or classifier',
                                dest="train_option", default='SQuAD')
        arg_parser.add_argument('--output-dir', help=' Dir to generate outputs', dest="output_dir", default='./output')
        arg_parser.add_argument('--config-file', help=' Json Config file', dest="config_file",
        ...
        arg_parser.add_argument('--use-tpu', help=' Use a TPU', dest="use_tpu", default="False")
        arg_parser.add_argument('--precision', help='precison fp32 or bfloat16 ', dest="precision", default='bfloat16')
        arg_parser.add_argument('--do-lower-case', help=' Use lowercase for data', dest="do_lower_case",
                                default="False")
        arg_parser.add_argument('--vocab-file', help=' Vocabulary file for BERT', dest="vocab_file",
                                default='vocab.txt')
        ...
        arg_parser.add_argument('--input-file', help=' Input file for pretraining', dest="input_file",
        ...
        arg_parser.add_argument('--accum_steps', help=' Steps before Gradient Accumulation ', dest="accum_steps",
                                type=int, default=1)
        ...
        benchmark_script = os.path.join(
            self.args.intelai_models, '../bert_large', self.args.mode, self.args.precision,
            run_script)
        ...
        self.cmd_args =  \
            ...
            " --bert_config_file=" + str(self.args.config_file) + eoo + \
            " --do_train=" + str(self.args.do_train) + eoo + \
            " --train_batch_size=" + str(self.args.batch_size) + eoo + \
            " --accum_steps=" + str(self.args.accum_steps) + eoo + \
        ...
        if self.args.train_option == "SQuAD":
            self.cmd_args = self.cmd_args + \
                ...
                " --num_train_epochs=" + str(self.args.num_train_epochs) + eoo + \
