...
def _generate_subtokens_with_target_vocab_size(
  ...
  if min_count is not None:
    tf.compat.v1.logging.info("Using min_count=%d to generate vocab with target size %d" %
  ...
  def bisect(min_val, max_val):
    ...
    tf.compat.v1.logging.info("Binary search: min_count=%d resulted in %d tokens" %
                    (cur_count, val))
